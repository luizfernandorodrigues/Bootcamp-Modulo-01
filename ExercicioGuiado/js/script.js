...
async function fetchCountries() {
  ...
  allCountries = json.map((country) => {
    ...
    return {
      ...
      formattedPopulation: fomatNumber(population),
      flag: flag,
    };
  });
  render();
...
function renderSummary() {
  ...
  totalPopulationList.textContent = fomatNumber(totalPopulation);
  totalPopulationFavorites.textContent = fomatNumber(totalFavorites);
}
function handleCountryButtons() {
  const countryButtons = Array.from(tabCountries.querySelectorAll('.btn'));
  const favorieButtons = Array.from(tabFavorites.querySelectorAll('.btn'));

  countryButtons.forEach((button) => {
    button.addEventListener('click', () => addToFavorites(button.id));
  });

  favorieButtons.forEach((button) => {
